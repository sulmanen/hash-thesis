...
For the ThingLink dataset the counts for detecting duplicates for modified sets from table \ref{modifiedimages} aren in table \ref{simplethinglinkresults.} The accuracy according to eq. \ref{accuracy} averaged over all modified sets (table \ref{modifiedimages}) with data from table \ref{simplethinglinkresults} per $D_H$ is in figure \ref{thinglinksimpletotalaccuracy}.

ROC space for $D_H \in{0,4,8,12}$ averaged over all modified sets (table \ref{modifiedimages}) for the ThingLink dataset is in figure \ref{thinglinksimpletotalroc}.
