...
Raw contingency table results for the Paintings dataset are in table \ref{simpleresults} for the Simple Hash, table \ref{dctresults} for the DCT Hash, and table \ref{siftresults} for Local Features. For the ThingLink dataset, table \ref{simplethinglinkresults} contains the Simple Hash results, DCT Hash results are in table \ref{thinglinkdctresults}. Local Features results are in table \ref{thinglinksiftresults}. By ROC analysis and calculating \emph{informedness} and \emph{markedness} (table) we can draw conclusions for a good $\Delta(q,d)$ for the perceptual hashes, and for a good $\mathcal{C}_s$ for the Local Features NDID.
