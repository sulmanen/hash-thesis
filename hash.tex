...
\subsection{Perceptual hashing vs. Local Features based-Classifier for Near Duplicate Image Detection}
\epigraph{Without data you're just another person with an opinion}{W. Edwards Deming}
To begin production implementation solving the near similar image matching problem with local-features we need to know if the new solution is an improvement over the existing what we call the simple hash and a version DCT hash. To what degree if any is object instance detection by local features an improvement over DCT-, and simple block mean- perceptual hashes for near duplicate image detection? To compare object instance recognition by local features to perceptual hashing we will run simulations.

We are faced with a binary classification problem the image is either knon to be in the database, and the known image is returned or it is not in the database. A set of images is needed, as well as well as modified sets described in table \ref{modifiedimages} and a set of images not modified but clearly not in the database, simulating scenarios typically encountered in duplicate image detection. We also test the classifiers with images known not to be in the database with a TN image set (table \ref{truenegatives}).

Receiver operating characteristics have been useful for visualizing classifier performance \cite{Fawcett2006}, however they were found not to be as precise and useful as PR curves by \cite{Davis2006}. We will construct PR curves for the simple hash, DCT-hash and large scale image retrieval by local features.

\def\arraystretch{1.5}
\begin{table}[htb]
\caption{Test image sets taken from the "Recognition of object instances practical" \cite{Vedaldi2012} paintings image set. }
...
\def\arraystretch{1.5}
\begin{table}[htb]
\caption{True negatives data set taken from the oxford images in ''Recognition of object instances practical''.}
\label{truenegatives}
\begin{center}
\begin{tabular}{lp{0.5\linewidth}}
  Modification & Motivation \\
...
Paintings data set from the ''Recognition of object instances practical'' \cite{Vedaldi2012} of a total of 1708 images (black and white images are excluded) will be used due to ready scripts for the SIFT case.
...
To test for images not in the set we will pick 1708 images from the ''Object instance recognition practical'' Oxford data set and run those images against the paintings set. This will look for a $fp_{rate}$ rate with images that are not in the data set.
...
Unexpectedly, the $TN$ test set the SIFT-classifier performed the worst returning all false positives instead of misses. This was due to not setting a cutoff score meaning ''query image not found in database'', a TN classification. The data was then investigated for an optimal cutoff score $\mathcal{C}_s$. If returned matches from a search score less than $\mathcal{C_s}$, the image is classified as a $TN$. The max score from the $TN$-set (table \ref{truenegatives}) was 19 (fig. \ref{sifttnscores}). In order to determine optimum cutoff, simulations were run setting the cutoff score from 1 to 19 and finding the optimum score, which turned out to be 19, the maximum score in the $TN$ data set (table \ref{truenegatives})
