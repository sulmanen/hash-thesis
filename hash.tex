...
To find out, modified image sets in table \ref{modifiedimages} for the Paintings dataset and ThingLink dataset were queried against a database formed from the original images. The Three NDID systems-the Simple Hash-, the DCT Hash- and Local Features-based system were queried separately and the results counted.

What is a good cutoff score $\mathcal{C}_s$ for the Local Features NDID system to classify an incoming image a TN? With the aid of the Paintings dataset, a good cutoff score $\mathcal{C}_s$ for Local Features NDID was determined from ROC space (fig. \ref{figcutoffrocspace}). What is a good $D_H$ for the DCT- and Simple hashes for NDID? For the Simple Hash, good $D_H$ was deteremined from ROC space \ref{simpletotalroc} and accuracy diagram \ref{simpletotalaccuracy}. For the DCT Hash a good $D_H$ was also determined by plotting averaged results from table \ref{dctresults} ROC space (\ref{dcttotalroc}) and accuracy plot (fig. \ref{dcttotalaccuracy}).

As a result, for head to head NDID system comparison the Simple Hash used $D_H=4$ and the DCT-Hash used $D_H=8$. The cutoff score for Local Features NDID was $\mathcal{C}_s=19$ for the Paintings dataset. Based on these $\mathcal{C}_s$ and $D_H$, the systems were compared head to head for TPR (fig. \ref{tptotal}), FPR (fig. \ref{fptotal}) and TN (fig. \ref{tntotal}) for the paintings dataset from \cite{Vedaldi2012}. Precision-Recall values were calculated (table \ref{precisionrecall}) to aid in comparison.

With the aid of the ThingLink dataset consisting of images exported from ThingLink, good $D_H \in \{0,4,8,12\}$ were determined for the perceptual hashes. As with the paintings dataset, they were found (fig. \ref{thinglinksimpletotalroc}, \ref{thinglinksimpletotalaccuracy}) to be $D_H=4$ for the Simple Hash and $D_H=8$ for the DCT Hash. A good $\mathcal{C}_s=16$ for Local Features NDID was found by plotting the results in ROC space (fig. \ref{thinglinkfigcutoff}). Table \ref{modifiedimages} modifications were run with the three NDID systems and comparisons are available for TPR (fig. \ref{thinglinktptotal}), FPR (fig. \ref{thinglinkfptotal}) and TN (fig. \ref{thinglinktntotal}). The joint $\mathcal{P}-\mathcal{R}$ results are in table \ref{precisionrecall}.
