...
The a simple block mean based hash (Simple Hash) and a DCT based hash (DCT Hash) are perceptual hashes used by ThingLink in production. To find matches, the Hamming Distance ($\Delta(q,d)$), or the number of bit positions two perceptual hashes differ is used as the distance function. $\Delta(q,d)=10$ is used as recommended in \cite{Zauner2010}. However we do not know if this is the most effective setting.

Local Features have been successfully used for detecting near-duplicate images in \cite{Chum2008}, \cite{Chum2010}, \cite{Lee2010} and \cite{dong2012high}. Vedaldi et al. demonstrate large-scale image retrieval using Local Features detection and description and Bag-of-Visual-Words


However, is NDID by Local Features and large-scale image retrieval a better system compared to NDID by perceptual hashing in use at ThingLink today?
