...
The Fisher kernel brings the best of both pattern classification with generative or discriminative methods \cite{Jaakkola1999}. The Fisher kernel transforms a variable-size set of independent samples into a fixed size vector given samples follow a training set based generative model \cite{Jaakkola1999}. Variable-size accomodates for variable amounts of features extracted from an image. It was applied to image classification by Perronin et al. \cite{Perronnin2007} and improved in \cite{Perronnin2010}.

The idea behind the Fisher kernel is as follows. $p$ is a probability density function with parameters $\lambda$ a variable length vector samples $X$
\begin{equation}\label{fisherone}
\nabla_{\lambda}\,log\,p(X\mid\lambda)
\end{equation}
and it describes the direction in which parameters should be adjusted to best fit the model. One may characterize a signal with a gradient vector derived from a generative probability model $u_\lambda$ and to subsequently feed this representation to a discriminative classifier. \cite{Perronnin2007}

Let's follow \cite{Perronnin2010} and see how this idea can be used to process local descriptors. $X = {x_t, t = 1 \ldots T}$ is a set of T local feature descriptors of an image generated by a process modeled by $u_\lambda$.

\begin{equation}\label{fishervector}
 G_\lambda^X = \frac{1}{T}\nabla_\lambda\log u_\lambda(X)
\end{equation}

describes $X$ as gradient vector. The dimentionality of $G_\lambda^X$ depends only on the number of parameters in $\lambda$, not on $T$ \cite{Perronnin2010}. As different images have different numbers of interest points, this method allows for images in a database to be represented by codes of equal dimention.
...
is the gradient with respect to the standard deviation $\sigma_i$ (vector division is done term by term). \cite{Perronnin2010}
 ...
 Perronin applies images as signals and the generative model to be a visual vocabulary.
