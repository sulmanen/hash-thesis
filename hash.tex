...
\subsection{Scope}
This thesis considers near-duplicate image detection excluding all other uses of perceptual hashing, Local Features and large-scale image retrieval. Cryptographic hashing is not considered nor are any uses of perceptual hashing or other methods for digital watermarking. Object/image \emph{category} recognition is an interesting topic not in the scope of this thesis. Finding an improved perceptual hashing algorithm is outside the scope of this thesis. We are only interested in determining if the Local Features and large-scale image retrieval based NDID system is better to the currently used systems.
