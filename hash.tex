...
Principal Component Analysis (PCA) transforms observed data into linearly uncorrelated principal components.

Were assuming data $\boldsymbol{X} \in \Re^{p}$ to be zero mean, that is eq. \ref{zeromean} holds. Let the reconstruction of $\boldsymbol{X}$ in $\Re^{q}$ be

\begin{equation}\label{zeromean}
\frac{1}{N}\sum_n \boldsymbol{x}_n = 0
\end{equation}

Here it is used for dimentionality reduction. Given $\boldsymbol{X}$ with $n$ observations in $d$ dimensions PCA (eq. \ref{pca})
...
maps $d$ dimentions to $L$ dimentions which is also the number of principal components kept. $W$ is matrix of weights and $T$ are the principal components. The score matrix maximises variance of the preserved original data and minimising the total squared reconstruction error (eq. \ref{pcaerror})
