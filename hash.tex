...
\subsection{Web application response times}
The expectations of the customer using a web application set the standard for duplicate image detection response time. \cite{Nielsen1993}

\def\arraystretch{1.5}
\begin{table}[htb]
\caption{Web application response time. The 3 important limits. \cite{Nielsen1993}}
\label{webresponse times}
\begin{center}
  \begin{tabular}{rl}
    \hline
  response time (s) & user experience\\
  \hline
  0.1 & user feels system is acting instantaneously\\
  1 & user flow of thought uninterrupted\\
  10 & user loses attention\\
  \hline
\end{tabular}
...
Vector of locally aggregated descriptors (VLAD) attempts to optimize the three constraints in fig. \ref{nntradeoffs} for NN-search jointly. There are two limits to the number of images that can be indexed in practice. The memory footprint of one image descriptor and the search efficiency. VLAD was developed in order to represent local descriptors so image searches could be scaled with accuracy, efficiency and low memory consumption. Experiments show that VLAD significantly outperforms BOF for the same size. It provides good seach accuracy with reasonable vector dimentionality. It jointly optimizes reduction in dimensions with the effectivenss of indexing. \cite{Jegou2010}

VLAD works by
\begin{enumerate}
\item aggregating local descriptors into a vector
\item reducing the dimensionality of the vector
\item providing efficient indexing.
\end{enumerate}

and it draws inspiration from BoW and the Fisher Vector.
