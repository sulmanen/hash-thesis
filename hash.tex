...
\subsection{Hamming Distance $\Delta(q,d)$}
...
\subsection{$\mathcal{C}_s$ for Local Features and Bag-of-Visual-Words}
...
For future research the AKAZE M-LDB descriptor is a binary descriptor and can use $\Delta(q,d)$ as a distance function. It is readily pluggable into the ThingLink system. It would be tremendously interesting to run it through these simulations with varying $\Delta(q,d)$ and see how it performs.

We should increase simulation dataset size to 5 million images. It is expected to further increase the difference between the perceptual hashes and large-scale image retrieval for near-duplicate image detection.
