...
To find out, modified image datasets in table \ref{modifiedimages} for the Paintings dataset and ThingLink dataset were run against three NDID systems-the Simple Hash-, the DCT Hash- and Local Features-based system. For the Simple Hash (fig. \ref{simpletotalroc}) and DCT Hash (fig. \ref{dcttotalroc}), a good $D_H$ was determined by searching for duplicates for modified sets in table \ref{modifiedimages} for $D_H \in \{0,4,8,12\}$, averaging the results and plotting the average TPR and FPR per $D_H$ in ROC space.

What is a good $D_H$ for the DCT- and Simple hashes for NDID? What is a good cutoff score $\mathcal{C}_s$ for the Local Features NDID system to classify an incoming image a TN?

With the aid of the Paintings dataset, a good cutoff score $\mathcal{C}_s$ for Local Features NDID was determined from ROC space (fig. \ref{figcutoffrocspace}). Good $D_H$ was deteremined from ROC space \ref{simpletotalroc} and accuracy diagram \ref{simpletotalaccuracy} Simple Hash was compared with $D_H=4$, the DCT-Hash used $D_H=8$ and the cutoff score for Local Features NDID was $\mathcal{C}_s=19$. Based on these $\mathcal{C}_s$ and $D_H$, the systems were compared head to head for TPR (fig. \ref{tptotal}), FPR (fig. \ref{fptotal}) and TN (fig. \ref{tntotal}) for the paintings dataset from \cite{Vedaldi2012}. Precision-Recall values were calculated and are in table \ref{precisionrecall}.

For the ThingLink dataset consisting of images exported from the service, good $D_H \in \{0,4,8,12\}$ were determined for the perceptual hashes. As with the paintings dataset, they were found to be $D_H=4$ for the simple hash and $D_H=8$ for the DCT Hash. A good $\mathcal{C}_s=16$ for Local Features NDID was found by plotting the results in ROC space (fig. \ref{thinglinkfigcutoff}). Table \ref{modifiedimages} modifications were run with the three NDID systems and comparisons are available for TPR (fig. \ref{thinglinktptotal}), FPR (fig. \ref{thinglinkfptotal}) and TN (fig. \ref{thinglinktntotal}). The joint $\mathcal{P}-\mathcal{R}$ results are in table \ref{precisionrecall}.
