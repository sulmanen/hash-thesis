...
\begin{table*} \footnotesize
\caption{ $\Delta$ Precision $\mathcal{P}$ (eq. \ref{precision}), Recall (eq. \ref{recall}) $\mathcal{R}$ and $\mathcal{P}/\mathcal{R}$ for NDID by Local Features vs. Simple Hash and DCT Hash based on table \ref{informednessmarkedness}. Cases where perceptual hashing is better are highlighted.}
