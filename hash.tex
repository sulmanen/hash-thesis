...
UA   & facilitates end user interaction with Web content (Browser)\\
URI  & Uniform Resource Identifiers (URIs, aka URLs)\\
\end{tabular}
...
ThingLink requirements for a classifier are high true positive rate, a high true negative rate, and a low false positive rate of images at different scales. A customer is reporting 20\% to 30\% false negatives for dct hash and reporting that the simple hash is also missing too many images though we do not have the exat numbers. We attempt to widen the duplicate image definition from pixel matches to detecting the same scene or object in an image to reduce false negatives.
...
The simple hash (eq. \ref{simplehasheq}) nscales the image image to $8px * 8px$ grayscale before calculating the hash. The resulting hash has two parts. The most significant $64$ bits represent the image pixels and the least significatn $64bits$ represent the horizontal flip of the $8px * 8px$ grayscale image.
