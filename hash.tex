...
The simple block mean based hash (Simple Hash) and a DCT based hash (DCT Hash) are perceptual hashes used by ThingLink in production. To find matches, the Hamming Distance ($\Delta(q,d)$), the number of bit positions two perceptual hashes differ, is used as the distance function. $\Delta(q,d)=10$ is used currently as recommended in \cite{Zauner2010}. However we do not know if this is the most effective setting.
