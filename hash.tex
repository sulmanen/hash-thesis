...
Modified image datasets in tables \ref{modifiedimages} and \ref{truenegatives} were run against three NDID systems, the Simple Hash-, the DCT Hash- and Local Features-based system. For the perceptual hashes, optimal Hamming Distance was determined. For the Local Features NDID system, an optimal cutoff score $\mathcal{C}_s$ was determined. As expected, Local Features (SIFT) outperforms both perceptual hashes for all image sets with the Simple Hash for 53\% scale being an exception due to chance (fig. \ref{tptotal}).
...
TPR for Simple Hash NDID are over 90\% for most near duplicate sets from table \ref{modifiedimages} in figure \ref{tptotal}. The exceptions are adding a 10\% white border to the image, where $TPR < 5\%$. Another case with where $TPR < 5\%$ is the 10 \% rotation duplicates. Here the Simple Hash NDID simply breaks down.

Optimal Hamming Distance (section \ref{HammingSection}) is 4 inclusive for accuracy (fig. \ref{simpletotalaccuracy}) and is also optimal (out of those tested) in ROC space (fig. \ref{simpletotalroc}) compared to the other Hamming Distances tried. \cite{Zauner2010} cites 10 to be a good Hamming distance where to classify two images as equal, but for the paintings dataset from \cite{Vedaldi2012} and NDID, the accuracy suffers after increasing above 4.

\begin{figure}[htb]
\begin{center}
\includegraphics[height=10cm]{figures/simpleTotalAccuracy}
\end{center}
\caption{Effect of Hamming Distance on total accuracy of Simple Hash duplicate detection.}
...
\end{center}
\caption{ROC space per Hamming Distance based on averaging results on all data. }
\label{simpletotalroc}
\end{figure}

\subsection{DCT Hash}
TPR and FPR for DCT Hash NDID are in figures \ref{tptotal} and \ref{fptotal} for all near duplicate datasets from table \ref{modifiedimages}. Most cases achieve $TPR > 90\%$ with 10\% white border, 10\% rotation and 10px crop near duplicates. For the 10\% white border and 10\% rotation near duplicates, the DCT Hash breaks down for NDID. The crop 10px case is interesting as the DCT Hash has $TPR \approx 70\%$, ~20\% worse than the Simple Hash.

The $ROC$ of a DCT Hash based matcher depends on the chosen Hamming Distance. The effect on setting Hamming Distance on DCT Hash NDID can be seen in fig. \ref{dcttotalaccuracy} and ROC per Hamming Distance in figure \ref{dcttotalroc}. Accuracy increases with increasing Hamming Distance from 4 to 8 and decreases strongly at 12. The ROC seems to suggest 4 is the optimal setting for the paintings dataset.

\begin{figure}[htb]
\begin{center}
\includegraphics[height=10cm]{figures/dctTotalAccuracy}
\end{center}
\caption{ Effect of Hamming Distance on total accuracy of DCT hash duplicate detection. }
...
\subsection{Local Features}
TPR and FPR for the Local Features NDID for the set can be found in figure \ref{tptotal} and \ref{fptotal} denoted as SIFT, due to the SIFT descriptor being used. The only miss for Local Features is a duplicate image in the database which results in a $FP$ being recorded depending on which of the duplicates takes top spot during sort at random. Local Features-Classifier outperforms the perceptual hashes for all test sets in aggregated results (fig. \ref{fptotal}) with 53\% scale duplicate dataset being the exception. Here Local Features ties with the Simple Hash.
