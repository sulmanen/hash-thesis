...
To what degree if any is object instance detection by local features an improvement over DCT-, and simple block mean based- perceptual hashes for near duplicate image detection? Simulations on the paintings dataset from \cite{Vedaldi2012} should answer this question. Modified image datasets in tables \ref{modifiedimages} and \ref{truenegatives} were run against three NDID systems-the Simple Hash-, the DCT Hash- and Local Features-based system. For the perceptual hashes, optimal Hamming Distance was determined. For the Local Features NDID system, an optimal cutoff score $\mathcal{C}_s$ was determined. As expected, Local Features (SIFT) outperforms both perceptual hashes for all duplicate sets for FPR excluding the 53\% scale duplicate set where the Simple Hash is equally good (fig. \ref{tptotal}).
...
TPR for Simple Hash NDID is over 90\% for most near duplicate sets from table \ref{modifiedimages} in figure \ref{tptotal}. The exceptions are adding a 10\% white border to the image, where TPR < 5\%. Another case with where TPR < 5\% is the 10 \% rotation duplicates. Here the Simple Hash NDID simply breaks down.

Optimal Hamming Distance (section \ref{HammingSection}) is 4 inclusive for accuracy (fig. \ref{simpletotalaccuracy}) as well as ROC space (fig. \ref{simpletotalroc}) compared to the other Hamming Distances tried. \cite{Zauner2010} cites 10 to be a good Hamming Distance for classification of two images as equal, but for the paintings dataset from \cite{Vedaldi2012} and NDID, the accuracy suffers after increasing above 4.

\begin{table*}
\caption{NDID by Simple Hash results for duplicate sets (table \ref{modifiedimages}) for Paintings data from \cite{Vedaldi2012}. The results are for }
\label{simpleresults}
\begin{center}
...
\subsection{Local Features}
TPR and FPR for the Local Features NDID for the set are in figures \ref{tptotal} and \ref{fptotal}. The Local Features NDID is denoted as SIFT, due to the SIFT descriptor used in the simulations. Figure \ref{tptotal} shows the TPR ~100\%. The single FP for Local Features NDID is a duplicate image in the database resulting in a $FP$. Finding the correct duplicate depends on the duplicate taking the top spot during sorting of similarity scores. Local Features NDID outperforms the perceptual hashes for all test sets in aggregated results (fig. \ref{fptotal}) with the exception of 53\% scale duplicate dataset. Here Local Features ties with the Simple Hash.

\begin{table}[htb]\footnotesize
\caption{ NDID by Local Features results for duplicate sets (table \ref{modifiedimages}) for Paintings data from \cite{Vedaldi2012}. }
\label{siftresults}
\begin{center}
...
\cmidrule{2-4} \cmidrule{6-8} \cmidrule{10-12} \cmidrule{14-16}
    ...
    53\% Scale   & 1 & 1694 & 13 &\phantom{abc} & 1 & 1694 & 13 &\phantom{abc} & 1 & 1694 & 13 &\phantom{abc} & 1 & 1694 & 13\\
    83\% Scale   & 1 & 1704 & 3 &\phantom{abc} & 1 & 1694 & 13 &\phantom{abc} & 1 & 1694 & 13 &\phantom{abc} & 1 & 1694 & 13\\
    Crop 10px    & 1 & 1704 & 3 &\phantom{abc} & 1 & 1694 & 13 &\phantom{abc} & 1 & 1694 & 13 & \phantom{abc} & 1 & 1694 & 13\\
    10\% Border  & 1 & 1704 & 3 &\phantom{abc} & 1 & 1694 & 13 &\phantom{abc} & 1 & 1694 & 13 &\phantom{abc} & 1 & 1694 & 13\\
    Rotate 10\%  & 1 & 1698 & 9 &\phantom{abc} & 1 & 1694 & 13 &\phantom{abc} & 1 & 1694 & 13 &\phantom{abc} & 1 & 1694 & 13\\
    Normalize    & 1 & 1702 & 5 &\phantom{abc} & 1 & 1694 & 13 &\phantom{abc} & 1 & 1694 & 13 &\phantom{abc} & 1 & 1694 & 13\\
    Saturate     & 1 & 1705 & 2 &\phantom{abc} & 1 & 1694 & 13 &\phantom{abc} & 1 & 1694 & 13 &\phantom{abc} & 1 & 1694 & 13\\
    \bottomrule
\end{tabular}
\end{center}
\end{table}
...
\section{Discussion}
To what degree if any is object instance detection by local features an improvement over DCT-, and Simple- perceptual hashes for NDID?

Evaluate the dataset. Evaluate the modified images.

Evaluate the method.

Fucking excellent. With the exception of 53 \% scale where it ties with Simple Hash.

Handling duplicate images in the database. This is a case for ThingLink

Why is that? object instance recognition shines in comparing the same image slightly modified because it was designed to match scenes from different images and to detect known scenes or objects in much more variant circumstances than near duplicate images. The SIFT descriptor is by definition scale and rotation invariant. It is capable of matching insituations of affine distortion, change in 3D camera location, noise and changes in lighting.\cite{Lowe2004}
...
1708 images is hardly ThingLink scale. There are 5 million images on ThingLink. VLAD is a bad approximation of the actual descriptors, is the accuracy enough?
