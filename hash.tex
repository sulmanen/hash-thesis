...
What is a good cutoff score to classify an image a true negative in near-duplicate image detection with Local Features? The Local Features query returns an image with a score describing how well the query image fits to the best mach in the database. $\mathcal{C}_s$ determines at which score the image is labeled a TN. For both the Paintings and the ThingLink datasets $C_s \rightarrow max(C_s^{TN})$ FP$\rightarrow 0$ logarighmically while the FN rises linearly (fig. \ref{figcutoff}, \ref{thinglinkfigcutoff}), it makes sense to set the score as high as possible picking from the TN dataset query scores. It is the maximum score in the TN dataset for both the ThingLink dataset and the Paintings dataset. For the Paintings dataset $\mathcal{C}_s=19$ and for the ThingLink dataset $\mathcal{C}_s=16$. Some of the success of the Local Features NDID against the perceptual hashes ahs to do with setting the score such that all images in the TN set are labeled correctly by the Local Features classifier. Much of the reason for the lesser perceptual hash performance in all but the 10\% rotation and 10\% white border case, is the TN performance, very apparent in the exploding FP from the TN dataset as $D_H\rightarrow12$ (tables \ref{simpleresults}, \ref{dctresults}).

In theory, can object instance recognition by local features be deployed at a scale of 5 million images at ThingLink for improved near-duplicate detection? Our simulation results prove that for a near duplicate definition in table \ref{modifiedimages}, Local Features NDID is a statistically significant improvement over the Simple- and DCT perceptual hashes for most cases. However if the expected set of duplicates does not include cropping, rotation and adding content to the image in the form of a border for example, it may be difficult to justfify the development cost and the extra computational resources needed for the improvements in table \ref{deltainformednessmarkedness}. A local features NDID system has been successfully implemented in \cite{dong2012high}. Our Local Features NDID simulation system uses SIFT descriptors with BoW, inverted index and geometric verification to ensure sanity of retrived results. The geometric verification in the query step for a duplicate using this system takes 99\% of the query time \ref{computationalcost}. It would be worth the while to compare filtering approach in \cite{dong2012high} to the simulation used in this paper as well as IVFADC system presented in \cite{Jegou2010} providing ~50ms queries into 5 Million images, a scale very suitable for ThingLink. Simulations in this paper were performed on a static database, which is not the case in a production environment. The real performance can only be tested reliably in a production environment.
