...
When $p$ is a pdf with parameters $\lambda$ variable length vector samples $X$

\begin{equation}\label{fisherone}
\nabla_{\lambda}\,log\,p(X\mid\lambda)
\end{equation}

which describes the direction in which parameters should be adjusted to best fit the model. This gradient vector can then be classified using a discriminative classifier. The Fisher information matrix $F_{\lambda}$
