...
With the aid of the paintings dataset, a good cutoff score $\mathcal{C}_s$ for Local Features NDID was determined from ROC space (fig. \ref{figcutoffrocspace}). The Simple Hash was compared with $D_H=4$, the DCT-Hash used $D_H=8$ and the cutoff score for Local Features NDID was $\mathcal{C}_s=19$. Based on these values, the systems were compared head to head using the determined $\mathcal{C}_s$ and $D_H$ for TPR (fig. \ref{tptotal}), FPR (fig. \ref{fptotal}) and TN (fig. \ref{tntotal}) for the paintings dataset from \cite{Vedaldi2012}. Precision-Recall values were calculated and are in table \ref{precisionrecall}.
