...
The normalized histogram near-duplicate case (fig. \ref{histogramlabel}) have the perceptual hashes perform best at $\Delta(q,d)=4$ for both datasets in both ROC space and PR plots. Near-duplicates with saturated colors (fig. \ref{saturatelabel}) $\Delta(q,d)=4$ is clearly closest to (0, 1) in ROC space and (1, 1) in the PR plot for both datasets. It's safe to say that for the saturated colors case, $\Delta(q,d)=4$ is optimal for both the Simple Hash and the DCT Hash.
