...
Computational resources cost money. CPU time and memory footprint of the hashes in the case of perceptual hashes and image descriptors for Local Features NDID. We will run one duplicate set and record the time it takes to calculate the descriptor/hash, the final memory footprint of the descriptor/hash and the query time for both the Paintings-, and the ThingLink datasets. The image databases are roughly $\frac{1}{3}$ in relative size so it should give insight into how computational cost of a single query varies with image database size.
...
Computational cost of Local Features-, Simple Hash- and DCT Hash NDID systems for 53\% scale duplicate sets for both the Paintings- and the ThingLink dataset.Matlab version R2016a on an Intel Xeon E31230 3.20GHz with 8GB RAM on a 4.4.0 x86\_64 GNU/Linux. The descriptor size is the size of a single index in the inverted index, which contains one double for each word in the visual vocabulary, which is $1*10^6$ words in our case.  is the size of the inverted index, idf, vocabulary and metadata combined. Local Features query time is represented as x ms query + y ms geometric verification.

\begin{table*} \footnotesize
  \caption{Computational cost. $t_f^{avg}$ is the average time to calculate a single image descriptor or hash value. $s_f$ is the size of the descriptor or hash in bits. $t_q^{avg}$ is the average query time from the database and $db$ is the database.}
