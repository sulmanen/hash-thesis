...
We do not know how varying the hamming distance affects classifier performance, we will run all sets for hamming distances of 0, 4, 8 \& 12 to find an optimal Hamming Distance. 10 was recommended by \cite{Zauner2010} but we want to know how these hashes perform.

Images with dimesions exceeding 32px are ignored by the Simple Hash and DCT Hash algorithms. Having visual metadata for small images is counterproductive so they are ignored. This is also the reason why smaller scales than 53\% and 84\% were not used as some of the dataset images were too small to process.
...
Simulation code for is heavily based on the code provided by ''Recognition of Object Instances Practical'' \cite{Vedaldi2012}. The Local Features-Classifier in this case makes use of the SIFT-descriptor (section \ref{SIFTSection}) and quantization of the descriptor into visual words.

\begin{figure}[htb]
\begin{center}
\includegraphics[height=8cm]{figures/vedaldigenindex}
\end{center}
...
\begin{figure}[htb]
\begin{center}
\includegraphics[height=8cm]{figures/vedaldiquery}
\end{center}
