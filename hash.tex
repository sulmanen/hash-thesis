...
Receiver operating characteristic \(ROC\) curves visualize classifier performance by plotting FPR to TPR. The ideal system lies at $(0, 1)$ while $(1, 0)$ represents a system that fails at classifying everything. A random classifier will land somewhere on the positive diagonal as TPR = FPR.
...
Inverse Recall = TN / (TN + FP)
\end{equation}

\begin{equation}\label{inverseprecision}
Inverse Precision = TN / (FN + TN)
\end{equation}
...
\emph{rp} stands for Prevalence of positive cases. \emph{pp} on the other hand is the Bias of the model \cite{lafferty2001conditional}.
