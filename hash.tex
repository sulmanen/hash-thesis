...
Principal Component Analysis (PCA) transforms observed data into linearly uncorrelated principal components. Here it is used for dimentionality reduction. Given $\boldsymbol{X}$ with $n$ observations in $d$ dimensions PCA (eq. \ref{pca})

\begin{equation} \label{pca}
\boldsymbol{T}_L = \boldsymbol{X} \cdot \boldsymbol{W}_{L}
\end{equation}
