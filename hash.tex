...
The main drawback of KAZE features is that building the nonlinear scale space is computationally intense due to solving a large system of linear equations as there is no analytical solution to Additive Operator Splitting.

AKAZE features uses FED schemes for feature detection and description that reduce computation time of nonlinear scale space. AKAZE uses an LDB descriptor over M-SURF descriptor used in KAZE.
...
FED is equivalent to box filtering and can approximate Gaussian kernels with good quality and is easy to implement.
