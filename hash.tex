...
\begin{abstractpage}[english]
  Perceptual hashing outputs an image identifier that can be used for detecting images similar to the original image also known as near-duplicate images. ThingLink is a commercial image annotation service using perceptual hashing for near-duplicate image detection in order to find annotations of an original image to place on all near-duplicates. A customer is reporting 20-30\% of near-duplicate images missing annotations. The system is working as expected calling for improved near-duplicate image detection (NDID) methodology.  We apply Local Features and large-scale image retrieval to near-duplicate image detection. We use a Bag-of-Visual-Words based image retrieval system for near-duplicate detection by assuming the original image always has the highest score of the images returned by Bag-Of-Visual-Words query. Bag-of-Visual-Words always returns the best matching image regardless of how good the match. We employ a cutoff score and classify all queries returning images with scores below the cutoff as \emph{no duplicate found}. To determine if the new system is better than the perceptual hash-based systems, we generate seven different types of near-duplicate image sets from originals in two different image sets. The originals form the image database. We also use a set of images not in the database to determine how well the systems classify queries as \emph{no duplicate found}.
