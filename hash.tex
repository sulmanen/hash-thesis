...
Perceptual hashing creates an image identifier for use in finding images similar to the original image. These are called near-duplicate images. ThingLink is a commercial image annotation service and uses perceptual hashing for near-duplicate image detection in order to place annotations on all versions of an original annotated image. A customer is reporting 20-30\% of near-duplicate images missing annotations found in the original image. The system is working as expected calling for improved near-duplicate image detection (NDID) methodology.  We apply Local Features and large-scale image retrieval to near-duplicate image detection and show it to be superior to a simple block mean-based hash and a DCT-based perceptual hash. We show that a Bag-of-Visual-Words based image retrieal system can be tuned to near-duplicate detection by assuming the original to always be the highest scoring match to the near-duplicate images returned by Bag-Of-Visual-Words query. Bag-of-Visual-Words always returns the best match regardless of how good the match. we show a cutoff score can be used in order to reject a query. The cutoff score is optimal when set to the maximum score encountered during predicted negative image sets. The Hamming Distance, the number of bits by which the hashes differ is optimal all around by setting it to 4 for both perceptual hashes, although if the type of near-duplicate under scrutiny can be further used to choose a better value and depends on the type of hash and the type of duplicate.
...
\thispagestyle{empty}
Images are a cornerstone of the World Wide Web. ThingLink is a commercial service hosting over 5 million images enabling students, teachers, journalists, real-estate agents, just about anyone with web access, to annotate images such as the Bengtskär lighthouse in figure \ref{bengtskar}. Customers annotate images with ThingLink and place a ThingLink script on their website containing the annotated images. The script asks ThingLink with an image URL for annotations. The primary identifier for finding the correct image to annotations is the image Universal Resource Locator (URL). For different screen widths, scaled versions of the original images are served to viewers of the annotated image. Modified versions of the original image are also served from a different URL than the original image. These slightly modified versions are called \emph{near-duplicate images} and finding a near-duplicate image is called near-duplicate image detection (NDID) having many applications ranging from copyright enforcement to identifying image forgeries.  ThingLink uses a perceptual hash as a secondary identifier for linking annotations to the original image and slightly modified versions of the original image.
