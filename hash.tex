...
The simple block mean based hash (Simple Hash) and a DCT based hash (DCT Hash) are perceptual hashes used by ThingLink in production. To match near-duplicates to original images, Hamming Distance ($\Delta(q,d)$), the number of bit positions two perceptual hashes differ, is used as a distance function. $\Delta(q,d)=10$ is used currently as recommended in \cite{Zauner2010}. However we do not know if this is the most effective setting.
...
\item[--] In theory, can Local Features and large-scale image retrieval be deployed at a scale of 5 million images at ThingLink for improved near-duplicate detection?
\end{itemize}
