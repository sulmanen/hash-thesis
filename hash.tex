...
Modified image sets in tables \ref{modifiedimages} and \ref{truenegatives} were run against three classifiers, the Simple Hash-, the DCT-Hash- and the Local Features-classifiers. As expected, the Local Features-classifier outperforms both perceptual hashes for all image sets with the Simple Hash for 53\% scale being an exception (fig. \ref{tptotal}). The only miss for the SIFT-matcher is a duplicate image in the paintings data set and this is expected to be a challenge in designing a local features based classifier for large scale. Local Features-Classifier outperforms the perceptual hashes for all test sets in aggregated results (fig. \ref{fptotal}).
