...
To begin production implementation solving the near similar image matching problem with local-features we need to know if the new solution is an improvement over the existing what we call the simple hash and a version DCT hash. To what degree if any is object instance detection by local features an improvement over DCT-, and simple block mean- perceptual hashes for near duplicate image detection?

To compare object instance recognition by local features to perceptual hashing we will run simulations. We are faced with a binary classification problem the image is either knon to be in the database, and the known image is returned or it is not in the database. A set of images is needed, as well as well as a modified set described in table \ref{modifiedimages} simulating scenarios typically encountered in duplicate image detection.

Receiver operating characteristics have been useful for visualizing classifier performance \cite{Fawcett2006}. We will construct ROC graphs for the simple hash, DCT hash and large scale image retrieval with SIFT features.
