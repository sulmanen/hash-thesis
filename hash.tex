...
\begin{itemize}
\item[--] To what degree if any is Local Features and Bag-of-Visual-Words an improvement over DCT-, and simple block mean based- perceptual hashes for near-duplicate image detection?
...
\item[--] What is a good cutoff score $\mathcal{C}_s$ to return \emph{no near-duplicate match found} for near-duplicate image detection with Local Features and Bag-of-Visual-Words?
    \item[--] Is the highest scoring image with score $s \geq \mathcal{C}_s$ from a query into an inverted index of Bag-of-Visual-Words the near-duplicate match for the query?
...
Can Local Features be deployed at a scale of 5 million images at ThingLink for improved near-duplicate detection? Our simulation results prove that for a near duplicate definition in table \ref{modifiedimages}, Local Features NDID is a statistically significant improvement over the Simple- and DCT perceptual hashes for most cases. However, if the expected set of duplicates does not include cropping, rotation and adding content to the image in the form of a border for example, it may be difficult to justify the development cost and the extra computational resources needed for the improvements in table \ref{deltainformednessmarkedness}. A local features NDID system has been successfully implemented in \cite{dong2012high}. Local Features and Bag-of-Visual-Words does need more computational resources, but the increase is manageable \ref{computationalcost}.
