...
Local Features dominates both perceptual hashes in both ROC space and PR space. The DCT Hash is clearly an improvement over the Simple Hash in ROC space and only slightly in the PR plot. For the ThingLink Dataset, the ROC- and PR curves are nearly identical. There is once again a loss in precision and an increase in fpr for Local Features for the ThingLink Dataset compared to the Paintings Dataset. Here the perceptual hashes perform best at $\Delta(q,d)=4$ for both datasets in both ROC space and PR plots.
...
Once again Local Features offers steady performance for near-duplicate detection. The differences to the perceptual hashes are most apparent in the ThingLink Dataset ROC space aNd PR plot. The loss in precision and increased fpr over the Paintings Dataset is clearly visible. For this case the optimal $\Delta(q,d)=4$ is clearly closest to (0, 1) in ROC space and (1, 1) in the PR plot for both datasets. It's safe to say that for the saturated colors case, $\Delta(q,d)=4$ is optimal for both the Simple Hash and the DCT Hash. Distinctly, the DCT Hash offers superior performance compared to the Simple Hash in all plots, most clearly for the Paintings Dataset but also visible in the ThingLink Dataset plots.
