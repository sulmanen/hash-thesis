...
\thesistitle{Hajautusfunkion korvaaminen paikallisilla erityispiirteillä duplikaattikuvien tunnistuksessa}
\advisor{TkT Juho Kannala}
\degreeprogram{Electronics and electrical engineering}
\department{Radiotieteen ja -tekniikan laitos}
\professorship{Piiriteoria}
...
The Paintings Dataset from \cite{Vedaldi2012} contains images of homogenous quality and domain. ThingLink images are very heterogenous in quality and subject. An image dataset of 446 ThingLink images excluding duplicates is used to evaluate performance in the ThingLink domain. The images are varied from upclose food photos to broad landscapes. For the perceptual hashes, a good Hamming distance is determined. For Local Features NDID a good cutoff score is determined.
...
\end{center}
\caption{ Effects of cutoff on Local Features NDID performance for ThingLink dataset. At the top, FN increase very slowly and linearly as $\mathcal{C}_s \rightarrow 16$ for TN dataset (table \ref{truenegatives}). In the middle, for the averaged duplicates data (table \ref{modifiedimages}) FP decreases logarithmically as $\mathcal{C}_s \rightarrow 16$. At the bottom we can see accuracy increase as $\mathcal{C}_s \rightarrow 16$. $\mathcal{C}_s=16$ is optimal.}
