...
In the Local Features case, the query returns a list of images along with scores describing how many visual words an image in the database has in common with the query image. We choose the highest scoring image for consideration as a near-duplicate. What is a good cutoff score $\mathcal{C}_s$ reject an image, or in other words to classify an incoming image a TN? With the aid of the Paintings- and ThingLink Datasets, $\mathcal{C}_s$ was determined from ROC space and PR curves in fig. \ref{53roc} - \ref{Saturateprthinglink}.
